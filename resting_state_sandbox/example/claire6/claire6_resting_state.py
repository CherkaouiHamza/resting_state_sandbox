...
# package import
from resting_state_sandbox.config import root_dir, claire6_data_dir
...
dataset_dir = claire6_data_dir
jobfile = os.path.join(root_dir, "example", "claire6", "claire6_resting_state.ini")
...
data_filename = os.path.join(dataset_dir, "s006a1001_epi3p4mm.nii")
...
for i, cur_img in enumerate(iter_img(components_img)):
    if i in [19]: # the closest to the DMN
        display = plotting.plot_stat_map(cur_img, cut_coords=(0, -52, 24),
                                         title="IC %d" % i,)
display.savefig(os.path.join(output_dir, 'claire6_ica.png'))
...
display.add_markers(marker_coords=pcc_coords, marker_color='g',
                    marker_size=300)
display.savefig(os.path.join(output_dir, 'claire6_seed_based.png'))
